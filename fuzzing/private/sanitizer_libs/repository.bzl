...
def _find_llvm_config(repository_ctx):
    """Finds llvm-config next to the compiler specified in CC."""
...
def _sanitizer_libs_repository(repository_ctx):
    # Resolve all labels as early as possible to make the rule restarts less
    # costly.
    build_tpl_path = repository_ctx.path(
        Label("@rules_fuzzing//fuzzing/private/sanitizer_libs:BUILD.tpl"),
    )
